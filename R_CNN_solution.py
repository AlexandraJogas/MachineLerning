...
path = "C:\Users\Dom\Desktop\machine_learn_corse\targil33\Images"
annot = "C:\Users\Dom\Desktop\machine_learn_corse\targil33\Airplanes_Annotations"


#####################################################################################################

def get_iou(bb1, bb2):              #  to initialise selective search
    assert bb1['x1'] < bb1['x2']    #  initialising the function to calculate IOU (Intersection Over Union)
...
for e,i in enumerate(os.listdir(annot)):
    try:
        if i.startswith("airplane"):
            ...
            ss.setBaseImage(image)           # Loop over the image folder and set each image one by one as the base for selective search 
            ...
            fflag = 0
            bflag = 0
            for e,result in enumerate(ssresults): # Iterating over all the first 2000 results passed by selective search
...
for layers in (vggmodel.layers)[:15]:  # no update weights, take only 15 first layers, will not be training the weights of the first 15 layers 
...
X= vggmodel.layers[-2].output    # take the second last layer, remove the last layer of the VGG16 model which is made to predict 1000 classes
predictions = Dense(2, activation="softmax")(X)  # Dense(2)= two classes so the last dense layer of model should be a 2 unit softmax dense layer
...
class MyLabelBinarizer(LabelBinarizer):  # for one-hot encode the label
...
hist = model_final.fit_generator(generator= traindata, 
                                 ...
                                 validation_steps=2, c
                                 allbacks=[checkpoint, early])
    ...
    if i.startswith("4"):
        ...
        for e,result in enumerate(ssresults):
            if e < 2000:
                ...
                if out[0][0] > 0.70: # If the output of the model says the region to be a foreground image (i.e. airplane image)
